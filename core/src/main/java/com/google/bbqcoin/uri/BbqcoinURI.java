...
/**
 * <p>Provides a standard implementation of a Bbqcoin URI with support for the
 * following:</p>
 *
 * <ul>
 ...
 *
 * <ul>
 * <li>{@code bbqcoin:<address>}</li>
 * <li>{@code bbqcoin:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 * @see <a href="https://en.bbqcoin.it/wiki/BIP_0021">BIP 0021</a>
 */
public class BbqcoinURI {
    /**
     * Provides logging for this class
     */
    private static final Logger log = LoggerFactory.getLogger(BbqcoinURI.class);
    ...
    public static final String LITECOIN_SCHEME = "bbqcoin";
    ...
    /**
     * Constructs a new BbqcoinURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws BbqcoinURIParseException if the URI is not syntactically or semantically valid.
     */
    public BbqcoinURI(String uri) {
        this(null, uri);
    }
     ...
     * @throws BbqcoinURIParseException If the input fails Bbqcoin URI syntax and semantic checks.
     */
    public BbqcoinURI(NetworkParameters params, String input) {
        // Basic validation
        ...
        try {
            ...
            throw new BbqcoinURIParseException("Bad URI syntax", e);
        }

        // URI is formed as  bbqcoin:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form bbqcoin://address?....
        // We support both until Ben fixes his code.
        
        // Remove the bbqcoin scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : bbqcoin:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        // the & (%26) in Tom and Jerry gets interpreted as a separator and the label then gets parsed
        // as 'Tom ' instead of 'Tom & Jerry')
        String schemeSpecificPart;
        if (input.startsWith("bbqcoin://")) {
            schemeSpecificPart = input.substring("bbqcoin://".length());
        } else if (input.startsWith("bbqcoin:")) {
            schemeSpecificPart = input.substring("bbqcoin:".length());
        } else {
            throw new BbqcoinURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?");
        if (addressSplitTokens.length == 0 || "".equals(addressSplitTokens[0])) {
            throw new BbqcoinURIParseException("Missing address");
        }
        ...
        if (addressSplitTokens.length == 1) {
            ...
            if (addressSplitTokens.length == 2) {
                ...
                throw new BbqcoinURIParseException("Too many question marks in URI '" + uri + "'");
            }
        }
    ...
    private void parseParameters(NetworkParameters params, String addressToken, String[] nameValuePairTokens) {
        // Attempt to parse the addressToken as a Bbqcoin address for this network
        try {
            Address address = new Address(params, addressToken);
            putWithValidation(FIELD_ADDRESS, address);
        } catch (final AddressFormatException e) {
            throw new BbqcoinURIParseException("Bad address", e);
        }
        ...
        for (String nameValuePairToken : nameValuePairTokens) {
            ...
            if (tokens.length != 2 || "".equals(tokens[0])) {
                throw new BbqcoinURIParseException("Malformed Bbqcoin URI - cannot parse name value pair '" +
                        nameValuePairToken + "'");
            }
    ...
    private void putWithValidation(String key, Object value) {
        if (parameterMap.containsKey(key)) {
            throw new BbqcoinURIParseException("'" + key + "' is duplicated, URI is invalid");
        } else {
    ...
    /**
     * @return The Bbqcoin Address from the URI
     */
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("BbqcoinURI[");
        boolean first = true;
    ...
    public static String convertToBbqcoinURI(Address address, BigInteger amount, String label, String message) {
        return convertToBbqcoinURI(address.toString(), amount, label, message);
    }

    /**
     * Simple Bbqcoin URI builder using known good fields.
     * 
     * @param address The Bbqcoin address
     * @param amount The amount in nanocoins (decimal)
     * @param label A label
     * @param message A message
     * @return A String containing the Bbqcoin URI
     */
    public static String convertToBbqcoinURI(String address, BigInteger amount, String label, String message) {
        ...
        if (amount != null) {
            ...
            builder.append(Utils.bbqcoinValueToPlainString(amount));
