...
/**
 * <p>A Message is a data structure that can be serialized/deserialized using both the Bbqcoin proprietary serialization
...
public abstract class Message implements Serializable {
     ...
     * @param msg Bbqcoin protocol formatted byte array containing message content.
     * @param offset The location of the first msg byte within the array.
     * @param protocolVersion Bbqcoin protocol version.
    ...
    private void selfCheck(byte[] msg, int offset) {
        if (!(this instanceof VersionMessage)) {
            ...
            byte[] reserialized = bbqcoinSerialize();
    ...
    // These methods handle the serialization/deserialization using the custom Bbqcoin protocol.
    ...
    /**
     * Should only used by BbqcoinSerializer for cached checksum
     *
     * @return the checksum
     */
    byte[] getChecksum() {
    ...
    /**
     * Should only used by BbqcoinSerializer for caching checksum
     *
    ...
    /**
     * Returns a copy of the array returned by {@link Message#unsafeBbqcoinSerialize()}, which is safe to mutate.
     ...
     */
    public byte[] bbqcoinSerialize() {
        byte[] bytes = unsafeBbqcoinSerialize();
    ...
    /**
     * Serialize this message to a byte array that conforms to the bbqcoin wire protocol.
     * <br/>
     ...
     */
    public byte[] unsafeBbqcoinSerialize() {
        ...
        try {
            bbqcoinSerializeToStream(stream);
        } catch (IOException e) {
        ...
        if (parseRetain) {
            ...
            // which means another call to bbqcoinSerialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the bbqcoin wire format.
     *
     * @param stream
     * @throws IOException
     */
    final public void bbqcoinSerialize(OutputStream stream) throws IOException {
    ...
    /**
     * Serializes this message to the provided stream. If you just want the raw bytes use bbqcoinSerialize().
     */
    void bbqcoinSerializeToStream(OutputStream stream) throws IOException {
        log.debug("Warning: {} class has not implemented bbqcoinSerializeToStream method.  Generating message with no payload", getClass());
    }

    /**
     * This method is a NOP for all classes except Block and Transaction.  It is only declared in Message
     * so BbqcoinSerializer can avoid 2 instanceof checks + a casting.
     *
     * @return
     */
