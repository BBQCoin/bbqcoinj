 ...
 * the lightweight SPV mode, a {@link com.google.bbqcoin.store.BoundedOverheadBlockStore} may be a good choice.</p>
 *
 ...
 * {@link com.google.bbqcoin.store.H2FullPrunedBlockStore} implements full verification, which is equivalent to the
...
public abstract class AbstractBlockChain {
     ...
     *
     * Following this one down to the genesis block produces the story of the economy from the creation of Bbqcoin
    ...
    private boolean add(Block block, Set<Sha256Hash> filteredTxHashList, List<Transaction> filteredTxn, boolean tryConnecting)
        ...
        try {
            ...
            // article here for more details: http://code.google.com/p/bbqcoinj/wiki/SecurityModel
            try {
                block.verifyHeader();
        ...
        for (Transaction tx : transactions) {
            try {
                if (listener.isTransactionRelevant(tx)) {
                    if (clone)
                        tx = new Transaction(tx.params, tx.bbqcoinSerialize());
    ...
    private void checkTestnetDifficulty(StoredBlock storedPrev, Block prev, Block next) throws VerificationException, BlockStoreException {
        ...
        // There is an integer underflow bug in bbqcoin-qt that means mindiff blocks are accepted when time
        // goes backwards.
